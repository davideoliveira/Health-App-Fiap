...
const AtualizarTriagemScreen = ({ navigation, route }) => {

    const [ visivel, setVisivel ] = useState(false);
    const [hospital, setHospital]=useState(route.params.info.hospital)
    const [dataDaTriagem, setDataDaTriagem]=useState(route.params.info.dataDaTriagem)
    const [temperatura, setTemperatura]=useState(route.params.info.temperatura)
    const [pressao, setPressao]=useState(route.params.info.pressao)
    const [batimento, setBatimento]=useState(route.params.info.batimento)
    const [observacao, setObservacao]=useState(route.params.info.observacao)
        ...
        <View style={{backgroundColor: '#82B3A6', height: 800}}>
            ...
            <View style={styles.form}>
                ...
                <Pressable 
                    ...
                    onPress={() => alterar(route.params.info.id)}>
...
export default AtualizarTriagemScreen;
